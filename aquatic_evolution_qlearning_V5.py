...
class AquaticCreature:
    def __init__(self, num_segments=3, max_segments=3, min_segments=3):
...
# ==========================================================
def save_simulation(checkpoints, use_nn, target_angle, duration, filename=None):
    """Save simulation to a file"""
    if filename is None:
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        method = "nn" if use_nn else "tabular"
        filename = f"aquatic_sim_{method}_{timestamp}.pkl"
...
def list_saved_simulations():
    """List all saved simulation files in current directory"""
    files = [f for f in os.listdir('.') if f.startswith('aquatic_sim_') and f.endswith('.pkl')]
...
if __name__ == "__main__":
    ...
    if saved_files:
        print(f"\nFound {len(saved_files)} saved simulation(s):")
        for i, f in enumerate(saved_files[:5]):  # Show max 5
            print(f"  {i+1}. {f}")
        ...
        if choice.isdigit():
            ...
            if 0 <= idx < len(saved_files):
                print(f"\nLoading {saved_files[idx]}...")
                checkpoints, use_nn, angle, duration = load_simulation(saved_files[idx])
                speed = 1.0
                print(f"Loaded: {len(checkpoints)} checkpoints, Target: {angle}°, Duration: {duration}s")
                print(f"Method: {'Neural Network' if use_nn else 'Tabular'}")
                ...
                # Ask to save after visualization
                save_choice = input("\nSave this simulation? (y/n) [n]: ").strip().lower()
                if save_choice == 'y':
                    saved_file = save_simulation(checkpoints, use_nn, angle, duration)
    ...
    saved_file = save_simulation(checkpoints, use_nn, angle, duration)
    print(f"\nSimulation auto-saved to: {saved_file}")
    ...
    # Ask to save after visualization
    save_choice = input("\nSave simulation with custom name? (filename or 'n') [n]: ").strip().lower()
    if save_choice and save_choice != 'n':
        if not save_choice.endswith('.pkl'):
            save_choice += '.pkl'
        saved_file = save_simulation(checkpoints, use_nn, angle, duration, save_choice)
        print(f"Saved to: {saved_file}")
    elif save_choice != 'n':
        # Just confirm the auto-save
        print(f"Using auto-saved file: {saved_file}")
