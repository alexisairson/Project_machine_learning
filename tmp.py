...
class AquaticCreaturePhysics:
    ...
    def update(self, dt):
        ...
        total_spread = sum(abs(a) for a in self.angles)
        ...
        for i in range(self.num_segments):
            ...
            forward_dir = np.array([math.cos(self.base_orientation), 
                                    math.sin(self.base_orientation)])
        ...
        # 3. Apply Thrust
        self.head_vel = self.head_vel + thrust * dt
        ...
        if speed > 0.001:
            ...
            current_drag_coeff = self.water_drag * (1.0 + 2.0 * total_spread)
            
            drag = -current_drag_coeff * self.head_vel * speed
            self.head_vel = self.head_vel + drag * dt
        
        # 5. Update Position
        self.head_pos = self.head_pos + self.head_vel * dt
