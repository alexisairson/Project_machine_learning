...
def visualize(checkpoints, target_angle=0.0, duration=10.0):
    ...
    pygame.display.set_caption("Aquatic Evolution V15 - Visualizer (Fixed NN Learning)")
    ...
    thrust_btn = UIButton(570, 522, 70, 32, "THRUST", (80, 60, 120), toggle=True)
    drag_btn = UIButton(650, 522, 70, 32, "DRAG", (120, 60, 80), toggle=True)
    ...
    while running:
        ...
        # Creature - using segment_angles (orientation relative to base_orientation)
        if trajectory and playback_pos < len(trajectory):
            curr = trajectory[playback_pos]
            ...
            # Each segment_angles[i] is the orientation of segment i relative to base_orientation
            joints = [pos.copy()]
            seg_len = 15.0
            
            for i in range(num_seg):
                # Absolute world orientation of segment i
                seg_world_angle = base_orient + seg_angles[i]
                
                prev = joints[-1]
                # Segments extend backward from head
            ...
            # Eyes
            if len(joints) > 1:
                # Head direction is orientation of segment 0
            ...
            # Draw thrust vectors on articulations (joints) - CYAN color
            if thrust_btn.active and thrust_vectors:
                # Draw thrust vectors on each joint (articulation)
                for i, tv in enumerate(thrust_vectors):
                    if i < len(joints):
                        ...
                        if tv_mag > 0.01:  # Only draw if significant
                            # Scale and draw
                            end_x = jx + int(tv_arr[0] * vector_scale)
                            end_y = jy + int(tv_arr[1] * vector_scale)
                            
                            # Draw thrust vector (cyan color)
            ...
            # Draw total thrust on head - CYAN color, larger
            if thrust_btn.active and np.linalg.norm(total_thrust) > 0.01:
                ...
                # Total thrust is larger and more visible
            ...
            # Draw drag vectors on articulations (joints) - ORANGE color
            if drag_btn.active and drag_vectors:
                # Draw drag vectors on each joint (articulation)
                for i, dv in enumerate(drag_vectors):
                    if i < len(joints):
                        ...
                        if dv_mag > 0.01:  # Only draw if significant
                            # Scale and draw
                            end_x = jx + int(dv_arr[0] * vector_scale)
                            end_y = jy + int(dv_arr[1] * vector_scale)
                            
                            # Draw drag vector (orange color)
            ...
            # Draw total drag on head - RED/ORANGE color, larger
            if drag_btn.active and np.linalg.norm(total_drag) > 0.01:
                ...
                # Total drag is larger and more visible
        ...
        method = "NN (Flexible)" if use_nn else "Tabular"
        ...
        # Vector legend (show when either button is active)
        ...
        if not playing:
            screen.blit(tiny.render("Press PLAY | Scroll to zoom | Toggle THRUST/DRAG for vectors", True, (100, 120, 160)), (230, 495))
        
        screen.blit(tiny.render(f"Angles: {curr_segments} (per-segment)", True, (80, 100, 140)), (10, 575))
        
        # Version info
        screen.blit(tiny.render("V15 - Fixed NN Learning", True, (60, 80, 120)), (680, 575))
...
def list_saved_simulations(script_dir):
    ...
    for f in os.listdir(result_dir):
        # Match new naming pattern: simulation_tabular_*.pkl or simulation_nn_*.pkl
...
def load_simulation(filename):
    ...
    # Convert checkpoint dictionaries back to TrainingCheckpoint objects
    checkpoints = []
    for cp_dict in save_data['checkpoints']:
        # Convert trajectory positions back to numpy arrays
        trajectory = []
...
if __name__ == "__main__":
    ...
    print("\n" + "=" * 60)
    print("   AQUATIC CREATURE VISUALIZER - V15")
    print("   (Compatible with Fixed NN Learning)")
    print("=" * 60)
    ...
    if not saved_files:
        ...
        print("Please run 'aquatic_evolution_qlearning_V15.py' first.")
    else:
